...
function drawPetSprite(ctx, x, y, size, rot) {
    // Zeichnet das PNG-Sprite (neutraler Gesichtsausdruck) statt eines Emojis.
    if (typeof spriteDrawOnCanvas === 'function') {
        spriteDrawOnCanvas(ctx, typeof pet !== 'undefined' ? pet : null, x, y, size * 1.5, rot);
        return;
    }
    // Rückfall auf Emoji, falls das Sprite-Modul fehlt
    ctx.save();
    ctx.translate(x, y);
